...
-- Changelog v3.9:
-- [FIX] Box: GetBoundingBox() вызывается 1 раз при кэшировании игрока,
--       размер хранится в PlayerSizeCache[player] — не меняется при движении
-- [FIX] Strict режим: переписан полностью — не требует IsTackling,
--       работает только на физике (угол + TTC + скорость)
-- [FIX] DribbleDelayTime: min = -0.2 (отрицательное = упреждение)
-- [NEW] Kickoff/Penalty: CanTackle проверяет workspace.Bools.Kickoff и .Penalty
-- [NEW] PredictionBox: показывается всегда при наличии цели (не только в PerformTackle)
-- [OPT] CalcInterceptTime: компонентная арифметика, без Vector3 объектов
-- [OPT] Magnitude сравнения заменены на Magnitude² (без sqrt) везде где возможно
-- [OPT] Все Vector3 операции на hot-path через компоненты
...
local AutoTackleConfig = {
    ...
    OnlyPlayer           = true,
    RotationMethod       = "Snap",
    DribbleDelayTime     = 0,       -- v3.9: может быть -0.2 до +2.0
    ...
    ShowPredictionBox    = true,    -- v3.9: по умолчанию true
}
...
-- v3.9: кэш размеров тел — читается 1 раз при обнаружении игрока
-- Ключ: player, значение: {W=число, H=число, D=число, OffsetY=число}
local PlayerSizeCache    = {}

local SPECIFIC_TACKLE_ID = "rbxassetid://14317040670"

-- ============================================================
-- v3.9: РАЗМЕР ПЕРСОНАЖА — 1 РАЗ
-- Логика идентична ESP 3D Box:
-- берём Character:GetBoundingBox() — это AABB всего тела.
-- Возвращает CFrame центра и Vector3 размера.
-- offsetY = центр bounding box относительно HRP по Y
-- ============================================================
local function CachePlayerSize(player)
    ...
    if not ok or not bbSize then
        -- fallback: стандартный R15
        PlayerSizeCache[player] = { W = 3.6, H = 5.5, D = 3.6, OffsetY = 0.4 }
        return PlayerSizeCache[player]
    end

    -- Центр bounding box в мировых координатах
    local bbCenter = bbCF.Position
    -- offsetY = разница между центром BB и центром HRP по Y
    local offsetY  = bbCenter.Y - hrp.Position.Y

    PlayerSizeCache[player] = {
        W       = bbSize.X,
        H       = bbSize.Y,
        D       = bbSize.Z,
        OffsetY = offsetY,
...
-- v3.9: полностью компонентная — ноль Vector3 объектов внутри
local function GetPositionBasedVelocity(player)
    local h = AutoTackleStatus.TargetPositionHistory[player]
    if not h or #h < 2 then return 0, 0, 0 end  -- возвращаем компоненты
    local now = _tick()
    ...
    if not ox or (ox == nx and oy == ny) then
        if #h >= 2 then
...
-- v3.9: принимает компоненты, не Vector3
...
-- ============================================================
-- v3.9: 3D BOX
-- Размеры берутся из PlayerSizeCache (кэшированы 1 раз через GetBoundingBox)
-- OffsetY — реальное смещение центра тела от HRP
-- ============================================================
local BOX_EDGES = {
    {1,2},{2,3},{3,4},{4,1},
    {5,6},{6,7},{7,8},{8,5},
...
-- v3.9: принимает W/H/D и offsetY отдельно
local function GetBoxCorners(cf, w, h, d, offsetY)
    local hW = w*0.5; local hH = h*0.5; local hD = d*0.5
...
local function SetupGUI()
    ...
    for _, lb in ipairs(tackleL) do
        ...
        lb.Outline = true; lb.Center = true
        lb.Visible = false
    ...
    local function setY(lb, y) lb.Position = Vector2.new(cx, y) end
    setY(Gui.TackleWaitLabel,      oy); oy+=15
    setY(Gui.TackleTargetLabel,    oy); oy+=15
    setY(Gui.TackleDribblingLabel, oy); oy+=15
    setY(Gui.TackleTacklingLabel,  oy); oy+=15
    setY(Gui.EagleEyeLabel,        oy); oy+=15
    setY(Gui.CooldownListLabel,    oy); oy+=15
    setY(Gui.ModeLabel,            oy); oy+=15
    setY(Gui.ManualTackleLabel,    oy); oy+=15
    setY(Gui.PingLabel,            oy); oy+=15
    setY(Gui.AngleLabel,           oy); oy+=15
    setY(Gui.PredictionLabel,      oy); oy+=15
    setY(Gui.ServerPosLabel,       oy); oy+=15
    setY(Gui.KickoffLabel,         oy)
    ...
    setY(Gui.DribbleStatusLabel,   dy); dy+=15
    setY(Gui.DribbleTargetLabel,   dy); dy+=15
    setY(Gui.DribbleTacklingLabel, dy); dy+=15
    setY(Gui.AutoDribbleLabel,     dy)
...
-- ============================================================
-- v3.9: UpdateServerPosBox — использует наш собственный кэш (нет PlayerSizeCache для localPlayer)
-- Для себя просто используем стандартные значения
-- ============================================================
local function UpdateServerPosBox()
    ...
    -- Для себя: GetBoundingBox один раз
    local bbOk, bbCF, bbSz = pcall(function() return Character:GetBoundingBox() end)
    local bW, bH, bD, bOff = 3.6, 5.5, 3.6, 0.4
    if bbOk and bbSz then
        bW = bbSz.X; bH = bbSz.Y; bD = bbSz.Z
    ...
    if Gui.ServerPosLabel and DebugConfig.Enabled then
        ...
        local dp = serverPos - HumanoidRootPart.Position
...
-- ============================================================
-- v3.9: UpdatePredictionBox — показывается ВСЕГДА при наличии цели
-- Вызывается из Heartbeat loop, не только из PerformTackle
-- ============================================================
local function UpdatePredictionBox(ownerRoot, player)
    ...
    UpdateBoxLines(Gui.PredictionBoxLines, predictedCF,
        sz.W, sz.H, sz.D, sz.OffsetY, Color3.fromRGB(255,165,0))
end
...
local function Update3DCircle(circle, position, radius, color)
    ...
    local seg = #circle; local px, pz, py = position.X, position.Z, position.Y
    local invSeg = _2pi / seg
    local pts = {}
    for i = 1, seg do
        local a = (i-1)*invSeg
    ...
    for player, data in pairs(PrecomputedPlayers) do
        if data.IsValid and TackleStates[player] and TackleStates[player].IsTackling then
            ...
            if tr then
                ...
                local col = dist <= AutoDribbleConfig.DribbleActivationDistance
...
local function SetupDebugMovement()
    ...
    UserInputService.InputChanged:Connect(function(inp, gp)
        ...
        if inp.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            for lb, sp in pairs(startPos) do
                if lb.Visible then lb.Position = sp + delta end
            end
        end
    end)
...
local function IsPowerShooting(p)
    ...
    local pf = Workspace:FindFirstChild(p.Name)
...
local function UpdateDribbleStates()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        ...
        elseif st.IsProcessingDelay and not isDribNow then
            -- v3.9: DribbleDelayTime может быть отрицательным (упреждение)
            -- Если delay < 0, мы реагируем РАНЬШЕ конца дриббла (в UpdateDribbleStates
            -- мы этого не можем, но в проверке cooldown используем offset)
            local effectiveDelay = _mmax(AutoTackleConfig.DribbleDelayTime, 0)
            if now - st.LastDribbleEnd >= effectiveDelay then
                DribbleCooldownList[p] = now + 3.5
                st.IsProcessingDelay = false
            end
        end
    end
    -- v3.9: отрицательный delay = заносим в CooldownList ещё во время дриббла
    -- (за |delay| секунд до конца). Используем IsProcessingDelay = false + IsDribbling = true
    if AutoTackleConfig.DribbleDelayTime < 0 then
        local anticipate = -AutoTackleConfig.DribbleDelayTime  -- положительное число
        for p, st in pairs(DribbleStates) do
            if st.IsDribbling and not DribbleCooldownList[p] then
                -- Добавляем в cooldown list уже сейчас — такл будет раньше конца дриббла
                DribbleCooldownList[p] = now + 3.5
            end
        end
    end
...
-- ============================================================
-- PRECOMPUTE PLAYERS
-- v3.9: CachePlayerSize при первом обнаружении игрока
-- ============================================================
local function PrecomputePlayers()
    ...
    local maxD2 = AutoDribbleConfig.MaxDribbleDistance * AutoDribbleConfig.MaxDribbleDistance
    ...
    for _, p in pairs(Players:GetPlayers()) do
        ...
        -- v3.9: кэшируем размер тела при первом появлении
        if not PlayerSizeCache[p] then
            CachePlayerSize(p)
        end
        ...
        local pvMag2  = pvx*pvx + pvy*pvy + pvz*pvz
        local vx, vy, vz
        if alvMag2 >= pvMag2 then
            vx, vy, vz = alv.X, alv.Y, alv.Z
        else
        ...
        PrecomputedPlayers[p] = {
            Distance   = dist,
            IsValid    = true,
            IsTackling = TackleStates[p].IsTackling,
            RootPart   = root,
            VX = vx, VY = vy, VZ = vz,  -- v3.9: компоненты вместо Vector3
        }
...
-- ============================================================
-- v3.9: CanTackle — проверка Kickoff и Penalty
-- workspace.Bools.Kickoff и workspace.Bools.Penalty
-- ============================================================
local function CanTackle()
    -- v3.9: Kickoff / Penalty блокируют такл
    ...
    if wsBools then
        ...
        local kickoffActive = kickoff and kickoff:IsA("BoolValue") and kickoff.Value
        local penaltyActive = penalty and penalty:IsA("BoolValue") and penalty.Value
        if kickoffActive or penaltyActive then
            if Gui then
                local s = (kickoffActive and penaltyActive) and "Kickoff+Penalty"
                    or (kickoffActive and "Kickoff") or "Penalty"
    ...
    if Gui then
        Gui.KickoffLabel.Text  = "Kickoff/Penalty: OK"
        Gui.KickoffLabel.Color = Color3.fromRGB(0,255,0)
    end
    ...
    if wsBools and (wsBools.APG.Value == LocalPlayer or wsBools.HPG.Value == LocalPlayer) then
        return false, nil, nil, nil
    end
    ...
    local myBools = Workspace:FindFirstChild(LocalPlayer.Name) and Workspace[LocalPlayer.Name]:FindFirstChild("Bools")
...
local function PerformTackle(ball, owner)
    ...
    local startT = _tick(); local dur = 0.65
    local rotConn
    ...
    elseif method == "Legit" and ownerRoot and owner then
        ...
        rotConn = RunService.Heartbeat:Connect(function()
            if _tick()-startT < dur then
                local cp = HumanoidRootPart.Position
                HumanoidRootPart.CFrame = CFrame.new(cp) * (lockedCF - lockedCF.Position)
            else rotConn:Disconnect() end
        end)
    end
    ...
    AutoTackleStatus.Connection = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            if not canT or not ball then
                ...
                -- v3.9: всё равно показываем prediction box если есть цель
                if owner then
            ...
            if owner then
                local oRoot = owner.Character and owner.Character:FindFirstChild("HumanoidRootPart")
                -- v3.9: prediction box ВСЕГДА при наличии цели
                if oRoot then
                    UpdatePredictionLabel(oRoot, owner)
...
AutoTackle.Stop = function()
    ...
    for _, c in pairs(AutoTackleStatus.TargetCircles) do
        for _, ln in ipairs(c) do ln:Remove() end
    end
...
-- ==========================================================
-- AUTODRIBBLE MODULE — v3.9
-- STRICT режим полностью переписан:
--   - НЕ требует IsTackling (работает независимо от анимации)
--   - Только физика: угол вектора скорости к игроку + TTC + минимальная скорость
--   - Free: старое поведение (IsTackling обязателен в ActivationDistance)
-- ==========================================================
...
-- v3.9: EMA сглаживание скоростей
local _velSmooth = {}
...
local function GetSmoothedVelocity(player, vx, vy, vz)
    ...
    if not prev then
        _velSmooth[player] = { X=vx, Y=vy, Z=vz }
        return vx, vy, vz
    end
...
-- ============================================================
-- v3.9: ShouldDribbleNow — REBUILT Strict
--
-- Strict: чисто физическая проверка, без требования IsTackling
--   Условия: скорость >= 2.5, угол <= MinAngleForDribble, TTC < 0.22s
--   Это убирает ложные срабатывания И отсутствие реакции когда IsTackling=false
--
-- Free: проверяет IsTackling в зоне активации (старое поведение)
-- ============================================================
local function ShouldDribbleNow(player, data)
    ...
    local speed2D_sq  = vx*vx + vz*vz  -- без sqrt

    -- Серверная позиция игрока
    local serverCF = GetMyServerCFrame()
    local msx, msz = serverCF.Position.X, serverCF.Position.Z

    local tp   = root.Position
    local ping = AutoTackleStatus.Ping
    -- Предсказанная позиция таклера
    local predTx = tp.X + vx*ping*1.5
    local predTz = tp.Z + vz*ping*1.5

    local toMeX = msx - predTx
    local toMeZ = msz - predTz
    local dist2D_sq = toMeX*toMeX + toMeZ*toMeZ
    local dist2D    = _msqrt(dist2D_sq)  -- нужен для TTC
    ...
    -- [STAGE 0] Вплотную — реагируем немедленно
    if dist2D < 3 then
        if Gui then Gui.AngleLabel.Text = string.format("⚡ POINT_BLANK d=%.1f", dist2D) end
        return true
    end

    local mode = AutoDribbleConfig.DribbleMode
    ...
    if mode == "Free" then
        ...
        if dist2D <= actD then
            -- В зоне активации — требуем IsTackling
            if not data.IsTackling then
                if Gui then Gui.AngleLabel.Text = string.format("FREE d=%.1f NO_TACKLE", dist2D) end
                return false
            end
        ...
        local minSpd2 = 2.0*2.0
        if speed2D_sq < minSpd2 then
        ...
        local relSpeedFree = relSpd + _mmax(myAp, 0)
        if relSpeedFree < 0.5 then return false end
        local ttc = dist2D / _mmax(relSpeedFree, 1)
        if Gui then Gui.AngleLabel.Text = string.format("FREE A=%.0f° t=%.2f d=%.1f", _mdeg(_acos(dot)), ttc, dist2D) end
        return ttc < 0.4

    -- ==========================================
    -- STRICT MODE — v3.9 REBUILT
    -- Не требует IsTackling вообще
    -- Чисто физика: скорость + угол + TTC
    -- ==========================================
    else
        -- Минимальная скорость 2.5 studs/s
        local minSpdS2 = 2.5*2.5
        if speed2D_sq < minSpdS2 then
        ...
        -- Угол: нормализуем вектор скорости и считаем dot
        local dot = _mclamp((vx*invSpd)*umX + (vz*invSpd)*umZ, -1, 1)

        -- Проверяем угол только при дистанции >= 5 (иначе угол нестабилен)
        if dist2D >= 5 then
            if dot < GetCosThreshold() then
                if Gui then
                    Gui.AngleLabel.Text = string.format("[S] BAD_A %.0f° d=%.1f", _mdeg(_acos(dot)), dist2D)
                end
                return false
            end
        end

        -- TTC: проекция скорости таклера на направление "ко мне"
        local approach = vx*umX + vz*umZ
        -- Учитываем нашу скорость (убегание снижает TTC)
        local mvx, _, mvz = GetMyVelocityFromHistory()
        local myAp = -(mvx*umX + mvz*umZ)
        local relSpeedS = approach + _mmax(myAp, 0)

        -- Минимальная скорость сближения
        if relSpeedS < 1.0 then
            if Gui then
                Gui.AngleLabel.Text = string.format("[S] diverge rSpd=%.1f", relSpeedS)
            end
            return false
        end

        local ttc = dist2D / _mmax(relSpeedS, 1)
        if Gui then
            Gui.AngleLabel.Text = string.format("[S] A=%.0f° v=%.1f t=%.2f d=%.1f",
                _mdeg(_acos(dot)), speed2D, ttc, dist2D)
        end
        -- Strict: строгое окно TTC
        return ttc < 0.22
    end
end
...
-- v3.9: грязный флаг кэша с пониженными порогами
local _dribCache = {
    ...
    local function DribbleLoop()
        ...
        for p, d in pairs(PrecomputedPlayers) do
            if d.IsValid then
                ...
                if d.Distance < minDist then
                    minDist = d.Distance; target = p; targetData = d
                end
            end
        end
        ...
        if Gui then
            ...
            Gui.DribbleTacklingLabel.Text = target
                and string.format("Nearest: %.1f", minDist) or "Nearest: None"
        end
        ...
        if IsDribbleCacheDirty(target, targetData) then
            ...
            _dribCache.lastPlayer = target
            _dribCache.lastIsTackling = targetData.IsTackling
            ...
            _dribCache.lastMyPos = { X=hp.X, Z=hp.Z }
        else
...
function AutoDribbleTackleModule.Init(UI, coreParam, notifyFunc)
    ...
    LocalPlayer.CharacterAdded:Connect(function(newChar)
        ...
        AutoTackleStatus.TargetCircles = {}
        MyPositionHistory = {}
        PlayerSizeCache   = {}  -- v3.9: сброс кэша размеров при респавне
        _cachedAngleDeg   = -1
        _velSmooth        = {}
        CurrentTargetOwner = nil
