...
-- CHANGELOG v37.0:
-- [FIX]   Hook mode: mobile trajectory mismatch fixed — all args now remapped for mobile
-- [NEW]   AutoPickup: Height check slider (max ball Y for pickup trigger)
-- [NEW]   AutoPickup: Keybind toggle with MacLib UI sync
-- [NEW]   Visuals: 3D Distance Circle (Drawing-based, projected per-segment, correct radius)
...
local FIXEDPOWER        = 5.0
local GKREACHRADIUS     = 5.0
local GKREACHSPEED      = 12.0
local PASTGOALBASE      = 45
local PASTGOALCLOSE     = 70
local SPINTRICKDIST     = 72
local SPINSERVERMINDIST = 130
local AutoShootCurveDistanceLimit = 300
local RICOCHETMINDIST   = 72
local RICOCHETMAXDIST   = 175
local SPINCROSSBLOCKX   = 0.18
local AutoShootDerivMult = 4.5
local BALLRADIUS        = 1.168
local INSET             = BALLRADIUS * 0.6
local YTOPTARGET        = BALLRADIUS + 0.28
local YTOPSAFETY        = BALLRADIUS + 0.48
local YBOTINSET         = 0.22
local GOALDEPTHMIN      = 1.0
local GOALDEPTHMAX      = 1.00
local VISCURLMAX        = 1.45
local VISCURLPEAK       = 0.38
local VISFALLEARLYBIAS  = 0.92

-- ───────────────────── STATUS ─────────────────────
local AutoShootStatus = {Running=false, Connection=nil, RenderConnection=nil, InputConnection=nil}
local AutoPickupEnabled   = true
local AutoPickupDist      = 180
local AutoPickupSpoofValue = 2.8
local AutoPickupMaxHeight = 50      -- [NEW] max ball Y offset above ground for pickup
local AutoPickupStatus    = {Running=false, Connection=nil}
local ShowTrajectory      = true
local Show3DBoxes         = true
local ShowDistCircle      = true    -- [NEW] distance circle toggle
...
local DistCircleColor     = Color3.fromRGB(100, 200, 255) -- [NEW]
...
local function SetupGUI()
    ...
    for i, v in ipairs({Gui.Status,Gui.Dist,Gui.Target,Gui.Power,Gui.Spin,Gui.GK,Gui.Debug,Gui.Mode,Gui.Goal}) do
        v.Size = 18; v.Color = Color3.fromRGB(255,255,255)
        v.Outline = true; v.Center = true
...
local TRAJSEGMENTS    = 20
local TrajectoryLines = {}
local PeakCube        = {}
local StartCircle     = {}

-- [NEW] Distance circle: 64-segment polygon on the ground plane
local DISTCIRCLE_SEGS = 64
local DistCircleLines = {}

local function ApplyVisualStyles()
    for _, l in ipairs(TrajectoryLines) do
        l.Color = TrajectoryColor; l.Thickness = TrajectoryThickness
        l.Transparency = TrajectoryAlpha; l.ZIndex = 999; l.Visible = false
    end
    local function SC(cube, color, th)
        for _, l in ipairs(cube) do
            l.Color = color; l.Thickness = th or BoxThicknessThin
            l.Transparency = BoxAlpha; l.ZIndex = 1000; l.Visible = false
        end
    end
    ...
    for _, l in ipairs(StartCircle) do
        l.Color = StartCircleColor; l.Thickness = StartCircleThickness
        l.Transparency = StartCircleAlpha; l.ZIndex = 1000; l.Visible = false
    end
    -- [NEW] Distance circle style
    for _, l in ipairs(DistCircleLines) do
        l.Color = DistCircleColor; l.Thickness = 1.4
        l.Transparency = 0.35; l.ZIndex = 998; l.Visible = false
    end
end
...
local function InitializeCubes()
    for i = 1, 12 do
        ...
        TargetCube[i] = Drawing.new("Line"); GoalCube[i]   = Drawing.new("Line")
        NoSpinCube[i] = Drawing.new("Line"); PeakCube[i]   = Drawing.new("Line")
    end
    for i = 1, TRAJSEGMENTS do
    ...
    -- [NEW] init distance circle lines
    for i = 1, DISTCIRCLE_SEGS do
        if DistCircleLines[i] and DistCircleLines[i].Remove then DistCircleLines[i]:Remove() end
        DistCircleLines[i] = Drawing.new("Line")
    end
    ApplyVisualStyles()
end
...
local function DrawTrajectory(startPos, peakPos, endPos, peakFrac, spinStr)
    ...
    local tpeak = fwdTotal > 0.1 and math.clamp(peakHoriz/fwdTotal, 0.36, 0.44) or 0.40
    ...
    for i = 1, TRAJSEGMENTS do
        local t0, t1 = (i-1)/TRAJSEGMENTS, i/TRAJSEGMENTS
        local p0, p1 = point(t0), point(t1)
    ...
    for i = 1, #StartCircle do
        ...
        if i > 1 and prevOk and ok then
            ...
            l.From = prev2D; l.To = Vector2.new(s.X, s.Y); l.Visible = true
        elseif i > 1 then StartCircle[i-1].Visible = false end
    ...
    if prevOk and fv and fs.Z > 0 then
        ...
        l.From = prev2D; l.To = Vector2.new(fs.X, fs.Y); l.Visible = true
    else StartCircle[#StartCircle].Visible = false end
end

-- ───────────────────── DRAW ORIENTED CUBE ─────────────────────
local function DrawOrientedCube(cube, cframe, size)
    if not cframe or not size then
        for _, l in ipairs(cube) do l.Visible = false end; return
    end
    pcall(function()
        local h = size / 2
        local corners = {
...
-- ───────────────────── [NEW] DRAW DISTANCE CIRCLE ─────────────────────
-- Рисует горизонтальный круг радиуса R вокруг HRP на уровне земли.
-- Использует сегментную проекцию Camera:WorldToViewportPoint — без WorldToScreenPoint.
-- Каждый сегмент — отдельный Drawing.Line, что даёт чёткий результат без артефактов.
local function DrawDistanceCircle(centerPos, radius)
    if not ShowDistCircle or radius <= 0 then
        for _, l in ipairs(DistCircleLines) do l.Visible = false end
        return
    end
    local floorY = centerPos.Y - (Humanoid and Humanoid.HipHeight or 0) - (HumanoidRootPart and HumanoidRootPart.Size.Y * 0.5 or 0)
    local prev2D, prevOk = nil, false
    for i = 1, DISTCIRCLE_SEGS do
        local a = (i - 1) / DISTCIRCLE_SEGS * math.pi * 2
        local wp = Vector3.new(
            centerPos.X + math.cos(a) * radius,
            floorY + 0.15,  -- чуть над землёй чтобы не Z-fight
            centerPos.Z + math.sin(a) * radius
        )
        local sp, vis = Camera:WorldToViewportPoint(wp)
        local ok = vis and sp.Z > 0
        if i > 1 and prevOk and ok then
            local l = DistCircleLines[i - 1]
            l.From = prev2D; l.To = Vector2.new(sp.X, sp.Y); l.Visible = true
        elseif i > 1 then
            DistCircleLines[i - 1].Visible = false
        end
        prev2D, prevOk = Vector2.new(sp.X, sp.Y), ok
    end
    -- замыкаем круг: последний сегмент → первая точка
    local a0 = 0
    local wp0 = Vector3.new(centerPos.X + radius, floorY + 0.15, centerPos.Z)
    local sp0, vis0 = Camera:WorldToViewportPoint(wp0)
    if prevOk and vis0 and sp0.Z > 0 then
        local l = DistCircleLines[DISTCIRCLE_SEGS]
        l.From = prev2D; l.To = Vector2.new(sp0.X, sp0.Y); l.Visible = true
    else
        DistCircleLines[DISTCIRCLE_SEGS].Visible = false
    end
end

-- ───────────────────── HELPER: GetBallStartPos / GetVisualStartPos ─────────────────────
...
local function GetVisualStartPos()
    ...
    local footY   = rootPos.Y - (Humanoid and Humanoid.HipHeight or hrp.Size.Y * 0.5) - hrp.Size.Y * 0.9
...
local function UpdateGoal()
    local function getTeamGoalName()
    ...
    local _, enemyGoalName = getTeamGoalName()
    ...
    for _, part in ipairs(frame:GetChildren()) do
        if part:IsA("BasePart") then
            if part.Name == "Crossbar" then crossbar = part
            else
...
local function GetEnemyGoalie()
    ...
    local function addGoalie(part, name, isNPC)
        ...
        local localX   = local3.X; local localY = local3.Y
        ...
        table.insert(goalies, {hrp=part, localX=localX, localY=localY, distGoal=distGoal,
            name=name, isInGoal=isInGoal, isNPC=isNPC or false, hbRadius=hbRadius})
    end
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Team and player.Team.Name ~= myTeam then
            ...
            local hitbox = wModel and wModel:FindFirstChild("Hitbox")
            if hitbox and hitbox:IsA("BasePart") then addGoalie(hitbox, player.Name, false) end
        end
    end
    for _, npcName in ipairs({myTeam == "Away" and "HomeGoalie" or "Goalie", "Goalie", "HomeGoalie"}) do
        ...
        if hitbox and hitbox:IsA("BasePart") then addGoalie(hitbox, "NPC", true); break end
    end
    if #goalies == 0 then
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Team and player.Team.Name ~= myTeam then
                ...
                if char then
                    ...
                    if hum and hrp and hum.HipHeight > 4 then addGoalie(hrp, player.Name, false) end
                end
            end
        end
        local npcName = myTeam == "Away" and "HomeGoalie" or "Goalie"
        local npc = Workspace:FindFirstChild(npcName)
        if npc and npc:FindFirstChild("HumanoidRootPart") then addGoalie(npc.HumanoidRootPart, "NPC", true) end
    end
    if #goalies == 0 then
    ...
    if tr and dt > 0.02 and dt < 0.8 then vel = (best.hrp.Position - tr.pos) / dt end
    local assemblyVel = best.hrp.AssemblyLinearVelocity or Vector3.zero
    if assemblyVel.Magnitude > vel.Magnitude then vel = assemblyVel end
    ...
    GkTrack[best.name] = {pos=best.hrp.Position, time=now, vel=vel, hist=hist, localX=best.localX}
    ...
    if Gui then
        Gui.GK.Text = string.format("GK: %s%s X%.1f Y%.1f v%.1f b%.1f",
            best.name, isAggressive and "[RUSH]" or "", best.localX, best.localY, vel.Magnitude, biasX)
...
local function CalcLaunchDir(startPos, targetPos)
    ...
    if horizDist < 0.5 then return Vector3.new(0,1,0), 0, 0.1 end
    local V = AutoShootBallSpeed
    local k = AutoShootDragComp
    local t
    if k < 1e-5 then t = horizDist / V
    else local kd = math.min(k * horizDist / V, 0.990); t = -math.log(1 - kd) / k end
    local gravRamp   = 1 - math.exp(-t / 0.26)^2
    local baseComp   = 0.5 * GRAVITY * 0.19 / (k * V) * t * t * gravRamp
    local t2 = t * t; local s = t2 * t2 * 0.80 * 0.80
    ...
    local dir        = Vector3.new(targetPos.X, corrY, targetPos.Z) - startPos
    dir              = dir.Unit
    local cosAngle   = math.sqrt(dir.X*dir.X + dir.Z*dir.Z)
    local realT
    if k < 1e-5 then realT = cosAngle > 0.01 and horizDist / (V * cosAngle) or t
    else local kdCos = math.min(k * horizDist / math.max(V * cosAngle, 1), 0.990)
         realT = -math.log(1 - kdCos) / k end
...
local function GetTarget(dist, gkX, gkY, isAggressive, gkHrp, gkVel, gkIsNPC, gkHitRadius, gkBiasX, gkDriftX)
    ...
    local reactionBonus = math.min(GKREACHSPEED * approxT * 0.25, 3.0)
    local diveRange  = GKREACHRADIUS + reactionBonus
    ...
    for _, xf in ipairs(xPoints) do
        for _, yf in ipairs(yFracs) do
            ...
            local laneTightness = cornerness * math.clamp(0.18 + 0.58*centerness + 0.54*sameSide*playerSideFrac - 0.76*shotOpen, 0, 1)
            ...
            local yRange = math.max(0.5, GoalHeight - YTOPTARGET - YBOTINSET)
            local localY = YBOTINSET + yf * yRange
            local highFrac = math.clamp((yf - 0.60) / 0.40, 0, 1)
            localY = math.max(YBOTINSET, localY - (laneTightness + distTightBoost + longCornerInset) * 0.65 * highFrac * 0.56)
            local idealPos = GoalCFrame * Vector3.new(localX, localY, 0)
            local gkDistX = math.abs(gkX - localX); local gkDistY = math.abs(gkY - localY)
            ...
            if math.abs(pgkY - localY) < GKREACHRADIUS * 0.7 then score = score - 4.5 end
            ...
            local isFarCorner = (playerLocalX > halfW*0.2 and localX < -halfW*0.4) or (playerLocalX < -halfW*0.2 and localX > halfW*0.4)
            if gkIsNPC then
            ...
            if gkHrp then
                ...
                if gkOnPath > 1 and gkPerp < math.max(1.7, gkHitRadius * 0.70) then score = score - 10.0 end
            end
            ...
            if math.abs(gkX) > halfW * 0.55 and math.abs(localX) < halfW * 0.3 then score = score + 6.0 end
            ...
            if gkHrp then
                ...
                local gkBX    = gkProj.X; local gkBY = gkProj.Y
                ...
                local inShadow = math.abs(localX - gkBX) < hbHalfW and localY > gkBY - hbLow and localY < gkBY + hbHigh
                if inShadow then score = score - 14.0 end
            end
            local power    = FIXEDPOWER
            ...
            local canServerSpin  = not gkIsNPC and dist > SPINSERVERMINDIST and dist < AutoShootCurveDistanceLimit
            local isCenterCurl   = math.abs(localX) < halfW * 0.30 and (math.abs(gkX) < halfW * 0.14 or math.abs(gkBiasX) < halfW * 0.12)
            if canServerSpin and gkDist2D < 5.8 then
                spinDir = (gkX < localX) and "Right" or "Left"; score = score + 3.4 + (sameSideLane and 0.8 or 0.0)
            elseif canServerSpin and isCenterCurl then
                spinDir = (playerLocalX > 0) and "Left" or "Right"; score = score + 4.8 + (sameSideLane and 0.5 or 0.0)
            elseif canServerSpin and isCorner then
                spinDir = (localX < 0) and "Left" or "Right"; score = score + 2.4 + (sameSideLane and 1.0 or math.min(crossLaneOpen, 1) * 0.9)
            elseif canServerSpin then
                ...
                if fwdAngle < 42 then
                    spinDir = (localX < 0) and "Left" or "Right"; score = score + 1.8 + (sameSideLane and 0.6 or 0.0)
                end
            end
            local derivation = 0
            if spinDir ~= "None" then
                local dMult  = AutoShootDerivMult / (dist / 100)^2
                derivation   = (spinDir == "Right") and 1 or -1
                derivation   = derivation * dMult
                score        = score + math.min(dMult * 0.35, 1.5)
            end
            local safeEdge = GoalWidth/2 - BALLRADIUS
            local shootLocalX = math.clamp(localX + derivation, -safeEdge, safeEdge)
            local shootLocalY = localY
            if spinDir ~= "None" then
                local spinDrop = 0.18 + 0.34 * hFrac
                shootLocalY = math.max(YBOTINSET, localY - spinDrop)
            else
                ...
                shootLocalY = math.max(YBOTINSET, localY - noSpinDrop - extraFlatDrop)
            end
            local depthAlpha = 1 - math.exp(-dist / 95)
            local goalDepth  = GOALDEPTHMIN + (GOALDEPTHMAX - GOALDEPTHMIN) * depthAlpha
            ...
            if cosA > 0.01 and flightT > 0 then
                local hFactor = AutoShootDragComp > 1e-5 and (1 - math.exp(-AutoShootDragComp * flightT)) / AutoShootDragComp or flightT
            ...
            else aimPoint = shootPos end
            local flightTime = flightT
            local t40  = 0.40 * flightTime
            local hF40 = AutoShootDragComp > 1e-5 and (1 - math.exp(-AutoShootDragComp * t40)) / AutoShootDragComp or t40
            ...
            table.insert(candidates, {
                idealPos=idealPos, aimPoint=aimPoint, shootPos=shootPos, launchDir=launchDir,
                localX=localX, localY=localY, spin=spinDir, power=power, speed=speed,
                score=score, gkDist=gkDist2D, trajOk=trajOk, flightTime=flightTime,
                peakPos=peakPos, peakFrac=peakFrac, isTopCorner=isTopCorner,
                isLobShot=isLobShot, isRicochet=false,
            })
        end
    end
...
local function CalculateTarget()
    ...
    if Gui then Gui.Dist.Text = string.format("Dist: %.0f | Goal: %.0f×%.0f", dist, GoalWidth, GoalHeight) end
    ...
    local result = GetTarget(dist, gkX or 0, gkY or 0, isAggressive or false,
        gkHrp, gkVel, gkIsNPC or false, gkHitRadius or 2.2, gkBiasX or 0, gkDriftX or 0)
    if not result then
        TargetPoint = nil
    ...
    local closeExtra = CurrentDist < SPINTRICKDIST and PASTGOALCLOSE or 0
    local velMult    = (CurrentDist + PASTGOALBASE + closeExtra) / math.max(CurrentDist, 5)
    ShootVel         = ShootDir * FIXEDPOWER * 1400 * velMult
    CurrentSpin      = result.spin
    CurrentPower     = FIXEDPOWER
    local typePrefix = result.isRicochet and "RIC" or result.isLobShot and "LOB" or result.isTopCorner and "TC" or ""
    CurrentType      = string.format("%sX%.1f Y%.1f/%.1f gk%.1f", typePrefix, result.localX, result.localY, GoalHeight, result.gkDist)
    ...
    if Gui then
        ...
        local cosAd    = math.sqrt(result.launchDir.X^2 + result.launchDir.Z^2)
        local launchAngle = math.deg(math.atan(result.launchDir.Y / math.max(cosAd, 0.01)))
        local spinShown = result.spin == "Right" and "CurveL" or result.spin == "Left" and "CurveR" or "None"
        Gui.Power.Text = string.format("%.1f° t%.2fs %s Spin:%s", launchAngle, result.flightTime, result.trajOk and "OK" or "!steep", spinShown)
        Gui.Spin.Text  = string.format("V%.0f k%.2f drv%.1f curve%.0f d%.0f", AutoShootBallSpeed, AutoShootDragComp, AutoShootDerivMult, AutoShootCurveDistanceLimit, dist)
...
local function GetKeyName(key)
    ...
    local pretty = {LeftMouseButton="LMB",RightMouseButton="RMB",Space="Space",LeftShift="LShift",RightShift="RShift"}
...
local function UpdateModeText()
    ...
    if AutoShootMode == "Hook" then
        ...
        Gui.Mode.Text  = AutoShootManualShot and string.format("Mode: Packet [%s]", GetKeyName(AutoShootShootKey)) or "Mode: Packet Auto"
        Gui.Mode.Color = Color3.fromRGB(255, 255, 255)
    end
end

-- ───────────────────── HOOK MODE ─────────────────────
-- [FIX] На мобильных устройствах isMobile = true уходит в аргументах,
-- что меняет траекторию на сервере. Хукаем ВСЕ аргументы и подставляем
-- наши рассчитанные значения так же, как делает Packet режим.
-- Сигнатура FireServer: (dir, cframeOriginal, power, vel, isMobile, spinFlags, nil, flag)
-- a1=dir, a2=cframe, a3=power, a4=vel, a5=isMobile, a6=spinFlags, a7=nil, a8=flag

local HookState = {Active=false, Unhook=nil, orig=nil}
local function DisableHook()
    if HookState.Unhook then pcall(HookState.Unhook); HookState.Unhook = nil end
    HookState.Active = false; HookState.orig = nil
end
        ...
        return hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
            ...
            if method == "FireServer" and self == Shooter then
                if ShootDir then
                    ...
                    -- a5 = isMobile — принудительно ставим false, чтобы сервер
                    -- использовал PC-траекторию, которую мы и рассчитываем.
                    -- Все остальные trajectory-аргументы заменяем на наши.
                    return HookState.orig(self, ShootDir, a2, power, ShootVel, false, a6, CurrentSpin, a8, a9)
                end
            end
            return HookState.orig(self, ...)
        end))
    end)
    if ok and result then
        HookState.Active = true; HookState.orig = result
        HookState.Unhook = function() pcall(hookmetamethod, game, "__namecall", result) end
...
AutoShoot.Start = function()
    ...
    if AutoShootMode == "Hook" then EnableHook() else DisableHook() end
    AutoShootStatus.Connection = RunService.Heartbeat:Connect(function()
        ...
        if AutoShootMode == "Packet" and hasBall and TargetPoint and dist < AutoShootMaxDistance
            and not AutoShootManualShot and (tick() - LastShoot) > 0.3 then
    ...
    AutoShootStatus.InputConnection = UserInputService.InputBegan:Connect(function(inp, gp)
        ...
        if AutoShootMode == "Packet" then return end
        if inp.KeyCode == AutoShootShootKey or inp.UserInputType == AutoShootShootKey then DoManualShoot() end
    end)
    AutoShootStatus.RenderConnection = RunService.RenderStepped:Connect(function()
        ...
        else for _, l in ipairs(TargetCube) do l.Visible = false end end
        ...
        else for _, l in ipairs(GoalCube) do l.Visible = false end end
        ...
        else for _, l in ipairs(PeakCube) do l.Visible = false end end
        ...
        else for _, l in ipairs(NoSpinCube) do l.Visible = false end end
        -- [NEW] Distance circle visualization
        if showVisuals and ShowDistCircle and HumanoidRootPart then
            DrawDistanceCircle(HumanoidRootPart.Position, AutoShootMaxDistance)
        else
            for _, l in ipairs(DistCircleLines) do l.Visible = false end
        end
    end)
...
AutoShoot.Stop = function()
    ...
    for i = 1, TRAJSEGMENTS do if TrajectoryLines[i] and TrajectoryLines[i].Remove then TrajectoryLines[i]:Remove() end end
    for i = 1, #StartCircle do if StartCircle[i] and StartCircle[i].Remove then StartCircle[i]:Remove() end end
    for i = 1, DISTCIRCLE_SEGS do if DistCircleLines[i] and DistCircleLines[i].Remove then DistCircleLines[i]:Remove() end end
    StartCircle = {}
end

AutoShoot.SetDebugText = function(v) AutoShootDebugText = v; ToggleDebugText(v) end
...
local function AutoPickupTick()
    ...
    -- [FIX] Distance check
    if (HumanoidRootPart.Position - ball.Position).Magnitude > AutoPickupDist then return end
    -- [NEW] Height check: ball.Position.Y must be <= HRP.Y + AutoPickupMaxHeight
...
AutoPickup.Start = function()
    ...
    AutoPickupStatus.Connection = RunService.Heartbeat:Connect(function() AutoPickupTick() end)
end
...
local function SynchronizeConfigValues()
    ...
    local pairssync = {
        ...
        {uiElements.AutoPickupMaxHeight,         function(v) AutoPickupMaxHeight = v end}, -- [NEW]
    }
...
local function SetupUI(UI)
    ...
    if UI.Sections.AutoShoot then
        ...
        uiElements.AutoShootMode = UI.Sections.AutoShoot:Dropdown{
            Name = "Mode", Options = {"Packet", "Hook"}, Default = 1, Required = true,
            Callback = function(v)
                AutoShootMode = v; UpdateModeText()
                if uiElements.AutoShootKey then uiElements.AutoShootKey:SetVisibility(v == "Packet") end
                if AutoShootEnabled then AutoShoot.Stop(); AutoShoot.Start()
                elseif v == "Hook" then EnableHook() else DisableHook() end
            end,
        }
        ...
        uiElements.AutoShootKey = UI.Sections.AutoShoot:Keybind{
            ...
            Callback = function()
                if AutoShootMode == "Packet" then return end
                if AutoShootStatus.ManualShootFn then AutoShootStatus.ManualShootFn() end
            end,
            onBinded = function(v)
                if AutoShootMode == "Hook" then return end
                AutoShootShootKey = v; UpdateModeText()
            end,
        }
        UI.Sections.AutoShoot:Divider()
        uiElements.AutoShootMaxDist = UI.Sections.AutoShoot:Slider{
            Name = "Max Distance", Minimum = 50, Maximum = 300, Default = AutoShootMaxDistance, Precision = 1,
            Callback = function(v) AutoShootMaxDistance = v end,
        }
        uiElements.AutoShootCurveDistanceLimit = UI.Sections.AutoShoot:Slider{
            Name = "Curve Distance Limit", Minimum = 130, Maximum = 300, Default = AutoShootCurveDistanceLimit, Precision = 1,
            Callback = function(v) AutoShootCurveDistanceLimit = v end,
        }
        uiElements.AutoShootBallSpeed = UI.Sections.AutoShoot:Slider{
            Name = "Ball Speed", Minimum = 100, Maximum = 1000, Default = AutoShootBallSpeed, Precision = 1,
            Callback = function(v) AutoShootBallSpeed = v end,
        }
        UI.Sections.AutoShoot:SubLabel{Text = "400 studs/s. if ball too high → increase, ball too down → reduce"}
        uiElements.AutoShootDragComp = UI.Sections.AutoShoot:Slider{
            Name = "Drag Compensation", Minimum = 0, Maximum = 3, Default = AutoShootDragComp, Precision = 2,
        ...
        uiElements.AutoShootDerivMult = UI.Sections.AutoShoot:Slider{
            Name = "Derivation Mult", Minimum = 0.5, Maximum = 10.0, Default = AutoShootDerivMult, Precision = 2,
        ...
        -- [NEW] Distance circle toggle + color
        uiElements.ShowDistCircle = UI.Sections.AutoShoot:Toggle{
            Name = "Show Distance Circle", Default = ShowDistCircle,
            Callback = function(v)
                ShowDistCircle = v
                if not v then for _, l in ipairs(DistCircleLines) do l.Visible = false end end
            end,
        }
        uiElements.DistCircleColor = UI.Sections.AutoShoot:Colorpicker{
            Name = "Distance Circle Color", Default = DistCircleColor,
            Callback = function(v) DistCircleColor = v; ApplyVisualStyles() end,
        }
    ...
    if UI.Sections.AutoPickup then
        ...
        -- [NEW] Keybind toggle для AutoPickup с синхронизацией MacLib UI
        uiElements.AutoPickupKey = UI.Sections.AutoPickup:Keybind{
            Name = "Toggle Pickup Key", Default = Enum.KeyCode.Unknown,
            Callback = function()
                -- Toggled by key press — flip enabled state
                AutoPickupEnabled = not AutoPickupEnabled
                if uiElements.AutoPickupEnabled then
                    uiElements.AutoPickupEnabled:UpdateState(AutoPickupEnabled)
                end
                if AutoPickupEnabled then AutoPickup.Start() else AutoPickup.Stop() end
            end,
            onBinded = function(v)
                -- just save the keybind
            end,
            ForceAutoLoad = true,
        }
        UI.Sections.AutoPickup:SubLabel{Text = "Press key to toggle AutoPickup on/off"}
        UI.Sections.AutoPickup:Divider()
        uiElements.AutoPickupDist = UI.Sections.AutoPickup:Slider{
            Name = "Pickup Distance", Minimum = 10, Maximum = 300, Default = AutoPickupDist, Precision = 1,
            Callback = function(v) AutoPickupDist = v end,
        }
        -- [NEW] Height check slider
        uiElements.AutoPickupMaxHeight = UI.Sections.AutoPickup:Slider{
            Name = "Max Ball Height", Minimum = 0, Maximum = 200, Default = AutoPickupMaxHeight, Precision = 1,
            Callback = function(v) AutoPickupMaxHeight = v end,
        }
        UI.Sections.AutoPickup:SubLabel{Text = "Max Y offset of ball above player for pickup to trigger"}
        uiElements.AutoPickupSpoof = UI.Sections.AutoPickup:Slider{
            Name = "Spoof Value", Minimum = 0.1, Maximum = 5.0, Default = AutoPickupSpoofValue, Precision = 2,
...
function AutoShootModule.Init(UI, coreParam, notifyFunc)
    ...
    LocalPlayer.CharacterAdded:Connect(function(newChar)
        ...
        CurrentPeakPos = nil; LastShoot = 0; IsAnimating = false; CanShoot = true
        if AutoShootEnabled  then AutoShoot.Start()  end
        if AutoPickupEnabled then AutoPickup.Start() end
    end)
end
